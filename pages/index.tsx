...
export default function Home() {
  ...
  async function toggleSession() {
    if (!session) return
    await supabase.from('vote_sessions').update({ is_open: !session.is_open, closed_at: !session.is_open ? null : new Date().toISOString() }).eq('id', session.id)
    loadSession()
  }
  ...
  function generatePlan() {
    ...
    const sorted = [...items].sort((a, b) => b.vote_count - a.vote_count).slice(0, 20)
