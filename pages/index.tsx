...
export default function Home() {
  ...
  async function loadSession() {
    ...
    if (data) { setSession(data); loadItems(data.id); loadMyVotes(data.id) }
  }

  async function loadItems(sessionId: string) {
    const { data } = await supabase.from('snack_vote_counts').select('*').eq('session_id', sessionId).order('vote_count', { ascending: false })
    if (data) setItems(data as SnackItem[])
  }

  async function loadMyVotes(sessionId: string) {
    const fp = getVoterFingerprint()
    const { data } = await supabase.from('votes').select('snack_id').eq('session_id', sessionId).eq('voter_fingerprint', fp)
  ...
  async function toggleVote(item: SnackItem) {
    ...
    if (myVotes.has(item.id)) {
      await supabase.from('votes').delete().eq('snack_id', item.id).eq('voter_fingerprint', fp)
      setMyVotes(prev => { const s = new Set(prev); s.delete(item.id); return s })
      showToast('已取消投票')
    } else {
      const { error } = await supabase.from('votes').insert({ session_id: session.id, snack_id: item.id, voter_fingerprint: fp })
      if (!error) { setMyVotes(prev => new Set(prev).add(item.id)); showToast('投票成功！') }
    }
    ...
    <>
      ...
      <main className={styles.main}>
        ...
        <div className={styles.topbar}>
          ...
          <span className={styles.pill} style={{ background: session?.is_open ? '#EAF3DE' : '#FAEEDA', color: session?.is_open ? '#3B6D11' : '#854F0B' }}>
        ...
        <div className={styles.tabs}>
          {(['vote', 'results', 'add', 'admin'] as const).map(t => (
            <button key={t} className={`${styles.tab} ${tab === t ? styles.active : ''}`} onClick={() => setTab(t)}>
              {t === 'vote' && '投票'}{t === 'results' && '結果'}{t === 'add' && '新增品項'}{t === 'admin' && '管理'}
            </button>
          ))}
        </div>
        ...
        {tab === 'vote' && (
          <>
            ...
            {filtered.length === 0 ? (
              <div className={styles.empty}>尚無品項，<button className={styles.linkBtn} onClick={() => setTab('add')}>新增第一個</button></div>
            ) : (
                  ...
                  return (
                    <div key={item.id} className={`${styles.card} ${voted ? styles.voted : ''}`}>
                      ...
                      <div className={styles.cardBody}>
                        ...
                        <button className={`${styles.voteBtn} ${voted ? styles.voteBtnActive : ''}`} onClick={() => toggleVote(item)} disabled={!session?.is_open}>
                          {voted ? '✓ 已投票（點選取消）' : '投票支持'}
                        </button>
                        {item.url && <a href={item.url} target="_blank" rel="noopener noreferrer" className={styles.storeLink}>前往 {item.store} 查看 →</a>}
                      </div>
                    </div>
                  )
                })}
                ...
                <div key={item.id} className={styles.resultRow}>
                  ...
                  <div className={styles.resultInfo}>
                    ...
                    <div className={styles.resultSub}>
                      ...
                      <span style={{ marginLeft: 6, color: '#aaa', fontSize: 12 }}>NT${item.price ?? '—'}</span>
                    </div>
                  ...
                  <div className={styles.resultRight}>
                    ...
                    <div style={{ fontSize: 11, color: '#aaa' }}>票</div>
        ...
        {tab === 'add' && (
          <div className={styles.addBox}>
            <div className={styles.urlRow}>
              <input type="text" placeholder="貼上全聯 / 家樂福 / momo / 蝦皮 商品網址…" value={urlInput} onChange={e => setUrlInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && fetchProduct()} className={styles.urlInput} />
            ...
            {preview && (
              <div className={styles.previewCard}>
                <div className={styles.previewImg}>{preview.image_url ? <img src={preview.image_url} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <span style={{ fontSize: 32 }}>🛒</span>}</div>
                <div style={{ flex: 1 }}>
                  <div className={styles.formGrid}>
                    ...
                    <div><div className={styles.formLabel}>賣場</div><select value={formStore} onChange={e => setFormStore(e.target.value)}>{Object.keys(STORE_STYLE).map(s => <option key={s}>{s}</option>)}</select></div>
                  </div>
          ...
          <>
            {!adminUnlocked ? (
              <div className={styles.addBox}>
                ...
                <div className={styles.urlRow}>
                  <input type="password" placeholder="請輸入管理密碼" value={adminPw} onChange={e => setAdminPw(e.target.value)} onKeyDown={e => { if (e.key === 'Enter' && adminPw === ADMIN_PASSWORD) setAdminUnlocked(true) }} />
                  <button className={styles.fetchBtn} onClick={() => { if (adminPw === ADMIN_PASSWORD) setAdminUnlocked(true); else showToast('密碼錯誤') }}>確認</button>
                </div>
              </div>
            ) : (
              <>
                ...
                {items.map(item => (
                  <div key={item.id} className={styles.adminRow}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: 12, color: '#888' }}>{item.vote_count} 票 · NT${item.price ?? '—'} · {item.store}</div>
                    </div>
